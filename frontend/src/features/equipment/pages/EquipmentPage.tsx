...
import type { Equipment, EquipmentHistory, Failure, Intervention } from '@/shared/types';
...
export default function EquipmentPage() {
  ...
  const load = async () => {
    ...
    try {
      const r = await equipmentApi.list({ search, page: 0, size: 50 });
      setItems(r.content);
      setListStatus('ready');
    } catch {
      setItems([]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const result = await execute(async () => {
      if (editId) return equipmentApi.update(editId, form);
      return equipmentApi.create(form);
    }, {
  ...
  const handleDelete = async (id: string) => {
    ...
    const result = await execute(() => equipmentApi.delete(id).then(() => true), {
  ...
  const showHistory = async (equipment: Equipment) => {

    const data = await equipmentApi.history(equipment.id);
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      <EnterpriseModal
        ...
        footer={<EnterpriseButton variant="secondary" onClick={historyModal.close}>Fermer</EnterpriseButton>}
