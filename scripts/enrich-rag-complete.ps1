...
try {
    psql -h $dbHost -p $dbPort -U $dbUser -d $dbName -f "enrichment-equipment.sql"
...
try {
    psql -h $dbHost -p $dbPort -U $dbUser -d $dbName -f "enrichment-failures.sql"
...
try {
    psql -h $dbHost -p $dbPort -U $dbUser -d $dbName -f "enrichment-interventions.sql"
...
try {
    ...
    $loginResponse = Invoke-RestMethod -Uri "http://localhost:8080/auth/login" -Method POST -ContentType "application/json" -Body $loginBody -TimeoutSec 10
    $headers = @{
        "Authorization" = "Bearer $($loginResponse.token)"
        "Content-Type" = "application/json"
    }

    # Réindexer les interventions
    $reindexResponse = Invoke-RestMethod -Uri "http://localhost:8080/api/v1/admin/knowledge/reindex" -Method POST -Headers $headers -TimeoutSec 30
